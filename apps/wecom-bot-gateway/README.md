...
机器人已连接后，在群内 @机器人或在单聊中发送以下指令：

- `帮助`：显示指令说明。
- `客户 杭州`：按客户名称、电话、省份或城市查询客户。
- `到期`：显示已到期及未来 30 天内到期的客户。
- `任务`：显示未完成任务。
- `商机`：显示线索、已确认需求和报价阶段的商机。
- `统计`：显示客户、进行中商机和未完成任务总数。

未匹配上述指令的自然语言问题会调用 Workers AI 生成回复。网关绑定了 Cloudflare Workers AI，并在 Durable Object 中限制为每天最多 20 次 AI 调用；明确的 CRM 指令直接查询 D1，不会消耗 AI 额度。
