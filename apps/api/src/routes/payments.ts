...
paymentRoutes.put('/:id', async (c) => {
  ...
  if (!parsed.success) return c.json({ error: parsed.error.issues[0]?.message ?? '回款资料格式无效' }, 400)
  const paidAt = parseOptionalDate(parsed.data.paid_at)
  if (parsed.data.paid_at !== undefined && paidAt === undefined) return c.json({ error: '打款日期格式无效' }, 400)
  const actor = getAuthenticatedActor(c)
  if (!actor) return c.json({ error: '登录凭证无效' }, 401)
  const db = createDb(c.env.DB)
  const [authorized] = await db.select({ id: payments.id, contractId: payments.contractId, invoiceId: payments.invoiceId, paymentNumber: payments.paymentNumber, amountCents: payments.amountCents, status: payments.status, paidAt: payments.paidAt, note: payments.note, claimedBy: payments.claimedBy }).from(payments)
    .innerJoin(customers, eq(payments.customerId, customers.id))
    .where(and(eq(payments.id, c.req.param('id')), eq(customers.isDeleted, false), ownershipFilter(actor)))
    .limit(1)
  if (!authorized) return c.json({ error: '回款不存在或无权编辑' }, 404)
  if (parsed.data.invoice_id !== undefined && !await validateInvoiceForContract(db, parsed.data.invoice_id, authorized.contractId)) {
    return c.json({ error: '发票不存在或不属于该合同' }, 400)
  }
  ...
  const updates = {
    ...
    ...(parsed.data.invoice_id !== undefined ? { invoiceId: parsed.data.invoice_id } : {}),
