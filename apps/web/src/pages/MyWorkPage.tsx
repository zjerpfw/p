  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <Card className="gap-0 py-0"><CardHeader className="flex flex-col gap-3 border-b border-border px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><CardTitle>{taskStatus === 'Open' ? '待完成任务' : '已完成任务'}</CardTitle><div aria-label="任务状态" className="flex rounded-md border border-slate-200 bg-slate-50 p-1" role="tablist">{([{ value: 'Open', label: '待完成' }, { value: 'Completed', label: '已完成' }] as const).map((item) => <button aria-selected={taskStatus === item.value} className={`h-8 rounded px-3 text-xs font-medium transition-colors ${taskStatus === item.value ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`} key={item.value} onClick={() => setTaskStatus(item.value)} role="tab" type="button">{item.label}</button>)}</div></CardHeader><CardContent className="divide-y divide-border p-0">
