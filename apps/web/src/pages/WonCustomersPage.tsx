...
export default function WonCustomersPage() {
  ...
  return <section className="space-y-6">
    ...
    {wonCustomersQuery.data && <Card className="gap-0 overflow-hidden py-0"><CardContent className="divide-y divide-border p-0">{wonCustomersQuery.data.data.map((customer) => { const expiryInfo = expiryState(customer.saasExpireDate); return <Link className="block p-5 transition-colors hover:bg-slate-50" key={customer.id} to={`/customers/${customer.id}`}><div className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold text-slate-900">{customer.name}</h2><Badge tone={expiryInfo.tone}>{expiryInfo.label}</Badge>{customer.latestProductName && <Badge tone="info">{customer.latestProductName}</Badge>}</div><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-600"><span className="inline-flex items-center gap-1"><MapPinned aria-hidden="true" className="size-4 text-emerald-600" />{[customer.province, customer.city, customer.address].filter(Boolean).join(' ') || '地域待完善'}</span><span className="inline-flex items-center gap-1"><Phone aria-hidden="true" className="size-4 text-slate-400" />{customer.contactPhone ?? '未填写电话'}</span></div></div><div className="shrink-0 text-right text-xs text-muted-foreground"><p>归属：{customer.ownerName ?? customer.ownerId}</p>{customer.saasExpireDate && <p className="mt-1 inline-flex items-center gap-1"><CalendarClock aria-hidden="true" className="size-3.5" />到期 {format(new Date(customer.saasExpireDate), 'yyyy-MM-dd')}</p>}</div></div></Link> })}{wonCustomersQuery.data.data.length === 0 && <p className="py-14 text-center text-sm text-muted-foreground">当前筛选条件下暂无成交客户</p>}<PaginationControls onPageChange={setPage} page={wonCustomersQuery.data.page} total={wonCustomersQuery.data.total} totalPages={wonCustomersQuery.data.totalPages} /></CardContent></Card>}
  </section>
}
