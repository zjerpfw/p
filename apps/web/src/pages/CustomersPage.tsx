    ...
    <section className="space-y-3.5 md:space-y-6">
      ...
      <div className="hidden md:flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <Button disabled={selectedCustomers.length === 0} onClick={() => setBatchTaskOpen(true)} type="button" variant="outline">
            <ClipboardPlus aria-hidden="true" />批量建任务{selectedCustomers.length > 0 ? ` (${selectedCustomers.length})` : ''}
          </Button>
          {isAdmin && (
            <Button disabled={selectedCustomers.length === 0} onClick={() => setBatchTransferOpen(true)} type="button" variant="outline">
              <ArrowRightLeft aria-hidden="true" />批量转交{selectedCustomers.length > 0 ? ` (${selectedCustomers.length})` : ''}
            </Button>
          )}
          <Button onClick={() => setCustomerImportOpen(true)} type="button" variant="outline">
            <Upload aria-hidden="true" />导入客户
          </Button>
          <Button onClick={() => void exportCustomers()} type="button" variant="outline">
            <Download aria-hidden="true" />导出客户
          </Button>
          <Button className="bg-emerald-600 shadow-sm shadow-emerald-200 hover:bg-emerald-700" onClick={() => setDirectWonDialogOpen(true)} type="button">
            <Zap aria-hidden="true" />直接录入成交客户
          </Button>
          <Button className="shadow-sm shadow-indigo-200" onClick={() => setCreateDialogOpen(true)} type="button">
            <Plus aria-hidden="true" />新建客户
          </Button>
        </div>
      </div>
      ...
      {isMobile && (
        <div className="space-y-2.5">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="搜索客户、电话或联系人..."
                value={search}
              />
              {search && (
                <Button aria-label="清空搜索" className="absolute right-1 top-1 size-8" onClick={() => updateSearch('')} size="icon-xs" type="button" variant="ghost">
          ...
          <div className="flex overflow-x-auto gap-2 no-scrollbar py-0.5">
            <select
              aria-label="客户状态筛选"
              className="h-8 shrink-0 rounded-full border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 shadow-sm"
            ...
            <select
              aria-label="跟进状态筛选"
              className="h-8 shrink-0 rounded-full border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 shadow-sm"
              onChange={(event) => updateFollowUp(event.target.value as '' | 'stale')}
              value={followUp}
            >
              <option value="">全部跟进</option>
              <option value="stale">⚠️ 7天未跟进</option>
            </select>
      ...
      {isLoading && <div aria-label="正在加载客户" className="grid gap-2.5" role="status">{[0, 1, 2, 3].map((item) => <Card className="gap-0 py-0" key={item}><CardContent className="space-y-3 p-4"><div className="flex items-center justify-between gap-3"><Skeleton className="h-5 w-2/5" /><Skeleton className="h-5 w-16" /></div><Skeleton className="h-4 w-1/2" /><Skeleton className="h-4 w-4/5" /></CardContent></Card>)}</div>}
      ...
      {data && isMobile && (
        <div className="space-y-2.5">
          {data.data.map((customer) => {
            const followUpInfo = getFollowUpLabel(customer.lastActivityAt)
            return (
              <Card className="gap-0 border-slate-200/80 bg-white py-0 shadow-sm active:bg-slate-50 transition-colors" key={customer.id}>
                <CardContent className="space-y-2.5 p-3.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge className="shrink-0 text-[10px]" tone={getCustomerStatusTone(customer.status)}>
                      {getCustomerStatusLabel(customer.status)}
                    </Badge>
                  </div>

                  <div className="flex items-center justify-between border-t border-slate-100 pt-2.5 text-xs">
                    <Badge tone={followUpInfo.tone} className="text-[10px]">
                      {followUpInfo.label}
                    </Badge>
                    ...
                    <div className="flex items-center gap-2">
                      {customer.contactPhone && (
                        <a
                          ...
                          className="flex h-7 items-center gap-1 rounded-md border border-indigo-200 bg-indigo-50 px-2 text-indigo-700 active:bg-indigo-100 font-medium"
                        >
