...
export default function AuditLogsPage() {
  ...
  return <section className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold text-indigo-600">组织与权限</p><h1 className="mt-1 flex items-center gap-2 text-2xl font-semibold text-slate-900"><History aria-hidden="true" className="size-6" />操作日志</h1><p className="mt-1 text-sm text-muted-foreground">查看关键客户、商机、合同、发票、回款和任务操作记录。</p></div><div className="flex flex-wrap gap-2"><select aria-label="筛选操作对象" className="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700" onChange={(event) => updateEntityType(event.target.value)} value={entityType}><option value="">全部对象</option>{Object.entries(entityLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select aria-label="筛选操作类型" className="h-9 rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700" onChange={(event) => updateAction(event.target.value)} value={action}><option value="">全部操作</option>{Object.entries(actionLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div></div>
    ...
    </TableBody></Table><PaginationControls onPageChange={setPage} page={logsQuery.data?.page ?? page} total={logsQuery.data?.total ?? 0} totalPages={logsQuery.data?.totalPages ?? 1} /></CardContent></Card>
  </section>
}
