  ...
  return (
    <Dialog onOpenChange={onOpenChange} open={Boolean(deal)}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
        ...
        <DialogFooter><Button onClick={() => onOpenChange(false)} type="button" variant="outline">取消</Button><Button disabled={!expectedCloseDate || updateDeal.isPending} onClick={() => updateDeal.mutate()} type="button">{updateDeal.isPending ? '正在保存' : '保存修改'}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  )
}
