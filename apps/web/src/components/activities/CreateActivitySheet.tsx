...
export function CreateActivitySheet({ customerId, deals, open, onOpenChange, onCreated }: CreateActivitySheetProps) {
  ...
  return <Sheet onOpenChange={onOpenChange} open={open}>
    <SheetContent className="w-full gap-0 p-0 sm:max-w-lg">
      <SheetHeader className="border-b border-slate-200 px-6 py-5"><SheetTitle>完整跟进记录</SheetTitle><SheetDescription>记录沟通详情、定位打卡并可将附件关联到本次跟进。</SheetDescription></SheetHeader>
      <div className="flex-1 space-y-5 overflow-y-auto p-6">
        ...
        <div className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">高德地图定位打卡</p><p className="mt-1 text-xs text-muted-foreground">{location?.address ?? locationError ?? '尚未获取定位'}</p></div><Button disabled={isLocating || isConfigLoading || !isConfigured} onClick={locate} size="sm" type="button" variant="outline"><MapPin aria-hidden="true" />{isLocating ? '正在定位' : '获取位置'}</Button></div>{!isConfigured && !isConfigLoading && <p className="text-xs text-destructive">系统未配置地图密钥，请联系管理员。</p>}</div>
      ...
      <SheetFooter className="border-t border-slate-200 bg-white px-6 py-4 sm:flex-row sm:justify-end"><Button onClick={() => onOpenChange(false)} type="button" variant="outline">取消</Button><Button disabled={!notes.trim() || !type || createActivity.isPending} onClick={() => createActivity.mutate()} type="button"><Send aria-hidden="true" />{createActivity.isPending ? '正在保存' : '保存跟进记录'}</Button></SheetFooter>
    </SheetContent>
  </Sheet>
}
